...
exports.getAvailableManagers = exports.getProjectStats = exports.syncProjectProgressWithTasks = exports.updateProjectProgress = exports.removeProjectMember = exports.addProjectMembers = exports.deleteProject = exports.updateProject = exports.createProject = exports.getProjectById = exports.getProjects = void 0;
...
const updateProject = async (id, data) => {
    ...
    if (memberIds !== undefined) {
        if (memberIds.length > 0) {
            const employeeMembers = await Employee_model_1.default.find({ _id: { $in: memberIds } });
...
const syncProjectProgressWithTasks = async (projectId) => {
    await (0, database_1.default)();
    const totalTasks = await Task_model_1.default.countDocuments({ projectId });
    const doneTasks = await Task_model_1.default.countDocuments({
        projectId,
    ...
    await (0, exports.updateProjectProgress)(projectId, newProgress);
};
exports.syncProjectProgressWithTasks = syncProjectProgressWithTasks;
const getProjectStats = async () => {
    await (0, database_1.default)();
    const statsResults = await Project_model_1.default.aggregate([
        {
            $facet: {
                total: [{ $count: 'count' }],
                active: [
                    { $match: { status: { $in: [Project_model_1.ProjectStatus.PLANNING, Project_model_1.ProjectStatus.IN_PROGRESS] } } },
                    { $count: 'count' }
                ],
                completed: [
                    { $match: { status: Project_model_1.ProjectStatus.COMPLETED } },
                    { $count: 'count' }
                ],
                overdue: [
                    { $match: { deadline: { $lt: new Date() }, status: { $ne: Project_model_1.ProjectStatus.COMPLETED } } },
                    { $count: 'count' }
                ],
                projectsByStatus: [
    ...
    const statusCounts = (facet.projectsByStatus || []).reduce((acc, item) => {
        acc[item._id] = item.count;
        return acc;
    }, {});
    const priorityCounts = (facet.projectsByPriority || []).reduce((acc, item) => {
        acc[item._id] = item.count;
        return acc;
    }, {});
    return {
...
const getAvailableManagers = async () => {
    ...
    const eligibleUsers = await User_model_1.default.find({
        role: { $in: [User_model_1.Role.ADMIN, User_model_1.Role.HR_MANAGER, User_model_1.Role.MANAGER] },
        isActive: true
    }).select('_id').lean();
