...
exports.groupService = {
    ...
    updateGroup: async (groupId, employeeId, data) => {
        await (0, database_1.default)();
        ...
        if (!membership || (membership.role !== models_1.GroupRole.ADMIN && membership.role !== models_1.GroupRole.MODERATOR)) {
            throw new error_middleware_1.AppError('Only admins and moderators can update group settings', 403);
        }
        const updatedGroup = await Group_model_1.default.findByIdAndUpdate(groupId, { $set: data }, { new: true, runValidators: true });
        if (!updatedGroup) {
